...
class DhcpBasicPacket:
    ...
    def SetOption(self, name, value):
        ...
        # if name is a standard dhcp field
        if DhcpFields.has_key(name) :
            check_len = DhcpFields[name][1]
            if len(value) != check_len:
                sys.stderr.write( "SetOption: bad option %s length, must be %s but received %s\n" % (name, check_len, len(value)) )
                return False
            begin = DhcpFields[name][0]
            end = DhcpFields[name][0]+DhcpFields[name][1]
        ...
        sys.stderr.write( "SetOption: error : unknown option %s\n" % name)
        return False
